	...
	std::shared_ptr<SimplePixelShader> brdfTablePS)
{
	...
	IBLCreateConvolvedSpecularMap(fullscreenVS, convolvedSpecularPS);
	IBLCreateBRDFLookupTable(fullscreenVS, brdfTablePS);
}
...
Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Sky::GetBRDFLookupTable()
{
...
void Sky::IBLCreateIrradianceMap(std::shared_ptr<SimpleVertexShader> fullscreenVS, std::shared_ptr<SimplePixelShader> irradiancePS)
{
	...
	device->CreateShaderResourceView(
		...
		irradianceCubeMap.GetAddressOf());	// Member variable of the Sky class
...
void Sky::IBLCreateConvolvedSpecularMap(std::shared_ptr<SimpleVertexShader> fullscreenVS, std::shared_ptr<SimplePixelShader> convolvedSpecularPS)
{
	...
	mipLevels = max((int)(log2(textureSize)) + 1 - mipSkipLevels, 1);
	...
	texDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE; // Will be used as both
	texDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; 		// Basic texture format
	texDesc.MipLevels = mipLevels; 					// No mip chain needed
	texDesc.MiscFlags = D3D11_RESOURCE_MISC_TEXTURECUBE; 	// It's a cube map
	texDesc.SampleDesc.Count = 1; 				// Can't be zero
	...
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE; // Sample as a cube map
	srvDesc.TextureCube.MipLevels = mipLevels; 		// Only 1 mip level
	srvDesc.TextureCube.MostDetailedMip = 0;			// Accessing the first (and only) mip
	srvDesc.Format = texDesc.Format; 	// Same format as texture
	device->CreateShaderResourceView(
	...
	fullscreenVS->SetShader();
	convolvedSpecularPS->SetShader();
	convolvedSpecularPS->SetShaderResourceView("EnvironmentMap", skySRV.Get()); // Skybox texture itself
	convolvedSpecularPS->SetSamplerState("BasicSampler", samplerOptions.Get());
	...
	for (int i = 0; i < mipLevels; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			...
			D3D11_VIEWPORT vp = {};
			vp.Width = (float)pow(2, mipLevels + mipSkipLevels - 1 - i);
			...
			convolvedSpecularPS->SetFloat("roughness", i / (float)(mipLevels - 1));
			convolvedSpecularPS->SetInt("faceIndex", j);
			convolvedSpecularPS->SetInt("mipLevel", i);
			convolvedSpecularPS->CopyAllBufferData();
...
void Sky::IBLCreateBRDFLookupTable(std::shared_ptr<SimpleVertexShader> fullscreenVS, std::shared_ptr<SimplePixelShader> brdfTablePS)
{
	Microsoft::WRL::ComPtr<ID3D11Texture2D> brdfFinalTexture;
	// Create the final irradiance cube texture
	D3D11_TEXTURE2D_DESC texDesc = {};
	texDesc.Width = textureSize; 			// One of your constants
	texDesc.Height = textureSize; 			// Same as width
	...
	srvDesc.Format = texDesc.Format; // Same format as texture
	device->CreateShaderResourceView(
		brdfFinalTexture.Get(), &srvDesc, brdfLookupTable.GetAddressOf());
